...
# David Field
# 1/4/2024
clinePlot_WGS <- function(clineModel,modelSummaryInput,curveFit,cexMain=1.1,cexAxisPlot= 1.85,
            axisLineWdth=2.5,cexPoints=1.8,lwdCurve=4,refLocus=NA,refPoints=F,HeaderDetails=F,FigLabel=NA,multipleCurves=F,
            multiplePoints=F,HaplotypeData=NA,poolSeqClines,poolSeqStats,plotKeyGene,colourTable) {
  ...
  # poolSeqClines <- allChr_Clines_Chr5clines ; poolSeqStats<- poolSeqStatsKeep; colourTable<- colourChoice 
  # refLocus <- F; HaplotypeData <- NA; plotKeyGene <- T; multiplePoints <- F
    ...
    if (curveFit==T & as.numeric(as.vector(modelSummaryInput[thisRow,"N_total"]))!=0) {
      ...
      if (round(as.numeric(as.vector(modelSummaryInput[thisRow,"centre_accept"])),0)!=0) {
        ...
        if (HeaderDetails==T & thisRow==1) { 
          ...
          axis(2, at=c(0,0.5,1.0), labels=NA,col.axis="black", las=2.2,cex.axis=cexAxisPlot,lwd=axisLineWdth*1.2,pos=-500,tck = -.018)
          axis(1, at=seq(-500,25000,500), labels=NA,col.axis="black", las=1,cex.axis=cexAxisPlot,lwd=axisLineWdth,pos=-0.01, tck = -.01)
          axis(1, at=seq(0,25000,2000), labels=((seq(0,25000,2000)/1000)),col.axis="black", las=1,cex.axis=cexAxisPlot,lwd=0,pos=-0.12)
          axis(1, at=seq(0,25000,1000), labels=NA,col.axis="black", las=1,cex.axis=cexAxisPlot,lwd=axisLineWdth*1.2,pos=-0.01, tck = -.016)
          ...
          if (length(poolSeqClines)>0) {
            for (thisRowPoolSeq in 1:nrow(poolSeqClines)) {
              ...
              if (poolSeqClines[thisRowPoolSeq,"ColourGene"]==-9) {
                ...
                back_col_transparent <- rgb(128, 128, 128, maxColorValue = 255, alpha = 20)
                lines(x=poolSeqClineThisLocus[,"DistAlongTransect"],y=poolSeqClineThisLocus[,"p"],lty=1,lwd=0.7,col= back_col_transparent)
                #points(x=poolSeqClineThisLocus[,"DistAlongTransect"],y=poolSeqClineThisLocus[,"p"],pch=16,lwd=5,col=rgb(180, 180, 180, maxColorValue = 255, alpha = 150))
                
                }
              # ROS EL
              
              if (poolSeqClines[thisRowPoolSeq,"ColourGene"]=="ROS1" | poolSeqClines[thisRowPoolSeq,"ColourGene"]=="ROS2" | poolSeqClines[thisRowPoolSeq,"ColourGene"]=="ROS3") {
                rosCol_linked <- colourTable[colourTable[,"Region"]=="ROS","lineColour"]
                rosCol_linked_trans <- rgb(166, 86, 40, maxColorValue = 255, alpha = 150)
                lines(x=poolSeqClineThisLocus[,"DistAlongTransect"],y=poolSeqClineThisLocus[,"p"],lty=1,lwd=1,col=rosCol_linked_trans)
              }
              if (poolSeqClines[thisRowPoolSeq,"ColourGene"]=="ELUTA") {
                rosCol_linked <- colourTable[colourTable[,"Region"]=="ROS","lineColour"]
                rosCol_linked_trans <- rgb(166, 86, 40, maxColorValue = 255, alpha = 50)
                lines(x=poolSeqClineThisLocus[,"DistAlongTransect"],y=poolSeqClineThisLocus[,"p"],lty=1,lwd=1,col=rosCol_linked_trans)
              }
              if ((poolSeqClines[thisRowPoolSeq,"ColourGene"]=="ROSEA" | poolSeqClines[thisRowPoolSeq,"ColourGene"]=="ELUTA") & poolSeqClines[thisRowPoolSeq,"distanceToGene"]!="inGene") {
                rosCol_linked <- colourTable[colourTable[,"Region"]=="ROS","lineColour"]
                rosCol_linked_trans <- rgb(166, 86, 40, maxColorValue = 255, alpha = 50)
                lines(x=poolSeqClineThisLocus[,"DistAlongTransect"],y=poolSeqClineThisLocus[,"p"],lty=1,lwd=1,col=rosCol_linked_trans)
              }
              # RUBIA
              if (poolSeqClines[thisRowPoolSeq,"ColourGene"]=="RUBIA" & poolSeqClines[thisRowPoolSeq,"distanceToGene"]!="inGene") {
                rubCol_linked <- colourTable[colourTable[,"Region"]=="RUBIA","lineColour"]
                rubCol_linked_trans <- rgb(55, 126, 184, maxColorValue = 255, alpha = 80)
                lines(x=poolSeqClineThisLocus[,"DistAlongTransect"],y=poolSeqClineThisLocus[,"p"],lty=1,lwd=1.3,col=rubCol_linked_trans)
              }
              if (poolSeqClines[thisRowPoolSeq,"ColourGene"]=="RUBIA" & poolSeqClines[thisRowPoolSeq,"distanceToGene"]=="inGene") {
                rubCol <- colourTable[colourTable[,"Region"]=="RUBIA","lineColour"]
                rubCol_trans <- rgb(55, 126, 184, maxColorValue = 255, alpha = 255)
                lines(x=poolSeqClineThisLocus[,"DistAlongTransect"],y=poolSeqClineThisLocus[,"p"],lty=1,lwd=1.5,col=rubCol_trans)
              }
              # SULF
              if (poolSeqClines[thisRowPoolSeq,"ColourGene"]=="SULF") {
              ...
              if (poolSeqClines[thisRowPoolSeq,"ColourGene"]=="CREMOSA" & poolSeqClines[thisRowPoolSeq,"distanceToGene"]!="inGene") {
              ...
              # AURINA
              if (poolSeqClines[thisRowPoolSeq,"ColourGene"]=="AURINA") {
              ...
              # AURINA
              if (poolSeqClines[thisRowPoolSeq,"ColourGene"]=="AURINA" & poolSeqClines[thisRowPoolSeq,"distanceToGene"]!="inGene") {
              ...
              # FLAVIA
              if (poolSeqClines[thisRowPoolSeq,"ColourGene"]=="FLAVIA" & poolSeqClines[thisRowPoolSeq,"distanceToGene"]!="inGene") {
        ...
        if (multiplePoints==F) {
              ...
              pointCol_transparent <- adjustcolor(pointCol, alpha.f = 100)
              # 
              if (pointCol=="#A65628") {
                points(cbind(XCordinates,p_adj),cex=cexPoints,lwd=6,pch=21,col="black",bg=rgb(166, 86, 40, maxColorValue = 255, alpha = 130))
              }
              if (pointCol=="#377EB8") {
                points(cbind(XCordinates,p_adj),cex=cexPoints,lwd=6,pch=21,col="black",bg=rgb(55, 126, 184, maxColorValue = 255, alpha = 130))
              }
        }
        if (multipleCurves==F) {
          curve(cline_model_sig_symm_plot, 0, 24500, n=2001, add=TRUE, lwd=lwdCurve*1.2,lty=1, col="black")
        }
        if (multipleCurves==T) {
          if (nrow(modelSummaryInput)>1) {
            for (thisRow in 1:nrow(modelSummaryInput)) {
        ...
        if (unique(poolSeqClines[,"LG"]==5) & plotKeyGene ==T) {
          poolSeqClines_leftClines <- poolSeqClines[poolSeqClines[,"centre"]>9000 & poolSeqClines[,"centre"]<11000,]
          theseOnes <- which(poolSeqClines[,"distanceToGene"]=="inGene")
          RUBlociPools <- poolSeqClines[theseOnes,]
          RUBloc <- RUBlociPools[4,]
          # ROS1 - 52889078
          poolSeqClineThisLocus <- cbind(poolSeqStats[2:7,"pool"],poolSeqStats[2:7,"DistAlongTransect"],as.vector(unlist(RUBloc[,c(9:14)])))
          colnames(poolSeqClineThisLocus) <- c("pool","DistAlongTransect","p")
          poolSeqClineThisLocus <- as.data.frame(poolSeqClineThisLocus)
          poolSeqClineThisLocus[,"DistAlongTransect"] <- as.numeric(poolSeqClineThisLocus[,"DistAlongTransect"])
          poolSeqClineThisLocus[,"p"] <- as.numeric(poolSeqClineThisLocus[,"p"])
          poolSeqClineThisLocus <- poolSeqClineThisLocus[order(poolSeqClineThisLocus[,"DistAlongTransect"]),]
          lines(x=poolSeqClineThisLocus[,"DistAlongTransect"],y=poolSeqClineThisLocus[,"p"],lty=5,lwd=lwdCurve,col="black")
          points(x=poolSeqClineThisLocus[,"DistAlongTransect"],y=poolSeqClineThisLocus[,"p"],pch=23,col='black',bg=c(rgb(255,255,255,230,maxColorValue=255)),lwd=5,cex=cexPoints*1.2)
        }
        if (unique(poolSeqClines[,"LG"]==6) & plotKeyGene ==T) {
          #poolSeqClines_leftClines <- poolSeqClines[poolSeqClines[,"centre"]>9000 & poolSeqClines[,"centre"]<11000,]
          ROSlociPools <- poolSeqClines[which(poolSeqClines[,"distanceToGene"]=='inGene'),]
          ROSloc <- ROSlociPools[ROSlociPools[,"position"]==52889078,]
          # ROS1 - 52889078
          poolSeqClineThisLocus <- cbind(poolSeqStats[2:7,"pool"],poolSeqStats[2:7,"DistAlongTransect"],as.vector(unlist(ROSloc[,c(9:14)])))
          colnames(poolSeqClineThisLocus) <- c("pool","DistAlongTransect","p")
          poolSeqClineThisLocus <- as.data.frame(poolSeqClineThisLocus)
          poolSeqClineThisLocus[,"DistAlongTransect"] <- as.numeric(poolSeqClineThisLocus[,"DistAlongTransect"])
          poolSeqClineThisLocus[,"p"] <- as.numeric(poolSeqClineThisLocus[,"p"])
