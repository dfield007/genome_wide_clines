# Cline fit locus set
# For repeating cline fitting over a vector of locus names
# note this sets up the deme data for the locus set, so no need to make the deme data in advance
# David Field 01/4/2024

clineFit_locusSet <- function(inData,locusSet=locusSet,rep=1,thisTrail,thisPop,dpThresh,demeSize,minSample,
                              removePhenoMiss,clineModel="symm_5p",
                              thisGradient, thisIntercept, adjustXY,pointsCol,gridAdd,
                              ...
                              plotMarkDownZoomed=F,plotEastNorthing=T,refLocus=NA,refPoints=F,HeaderDetails=T,FigLabel=NA,HaplotypeData=F,colourChoice=colourChoice) {
  # c(((3900-2240)/(0-13500)),3900)
  
  # inData <- GenoEcol_Planoles
  # locusSet=c("s1187_290152", "ros_assembly_543443")
  # thisTrail <- "rep1"; clineModel="symm_5p"; rep=1; thisPop <- "Planoles"; dpThresh <- 0.6; demeSize=200;minSample=10;removePhenoMiss=F;thisGradient = -0.197037;thisIntercept = 4900; adjustXY=F; pointsCol=F; gridAdd=F; plotAllele1D=T;plotDem=c(3,3);addPie=T;plotAxisLab=F;
  # minDemeSize=10; demeMaxSize=100;plotPerp=T; returnFreq=1;burnin=0;iterations=2000;startParams=c(10000,3000,0.1,0.9,15);fileFormat="png"
  ...
  # 
  # locusSet <- c("s1187_290152", # CRE
  #               "s316_93292", "s316_257789", # FLA
  #               "s261_720757", # RUBIA
  #               "ros_assembly_543443","ros_assembly_715015") # ros & el
  # locusSet <- c("s1187_290152", # CRE
  #               "ros_assembly_543443") # ros & el
  
  
  
  DemeData <- DemeSetup2D(HZdata=inData,locusSet,demeSize=demeSizeSelect,minSample=10,
  ...
  for (thisLocus in 1:length(locusSet)) {
    ...
    if (locusName%in%names(DemeData)) {
      ...
      if (deltaP < dpThresh) {
        # plot cline
        clinePlot(clineModel="no fit",locusName,clineSummary=summaryModelsAllloci,clineFit=F,curveFit=F,cexMain=1.3,cexAxisPlot= 1.85,
                  axisLineWdth=2.5,cexPoints=1.8,lwdCurve=4,refLocus,refPoints,HeaderDetails,FigLabel,HaplotypeData, colourChoice)      
        # as pdf
        ...
        clinePlot(clineModel="not fit",locusName,summaryModelsAllloci,curveFit=F,cexMain=2.5,cexAxisPlot= 2.7,
                  axisLineWdth=3.8,cexPoints=3,lwdCurve=4.3,refLocus,refPoints,HeaderDetails,FigLabel,HaplotypeData, colourChoice)
        dev.off()
        # # as tiff
        # fileClineFitName <- paste(paste(paste(locusName,"ClineFit",sep="_"),thisTrail,sep="_"),".tiff",sep="")
        ...
        clinePlot(clineModel="no fit",locusName,summaryModelsAllloci,curveFit=F,cexMain=1.4,cexAxisPlot= 2,
                  axisLineWdth=2,cexPoints=1.8,lwdCurve=4,refLocus,refPoints,HeaderDetails,FigLabel,HaplotypeData, colourChoice)
        dev.off()
        obsVals_thisLocus <- NULL
        curveFit <- NULL
        # update cline fits to file
        fileName <- paste(paste("summaryModelsAllloci",thisTrail,sep="_"),".csv",sep="")
      ...
      if (fileFormat=="eps") {
        fileTraceName <- paste(paste(paste(locusName,"Trace",sep="_"),thisTrail,sep="_"),".eps",sep="")
        cairo_ps(fileTraceName,width = 24, height = 9.8, pointsize = 14,antialias="default")
        tracePlot(SANNfit_thisLocus,locusName,acceptRate)
        dev.off()
      }
      if (fileFormat=="png") {
        fileTraceName <- paste(paste(paste(locusName,"Trace",sep="_"),thisTrail,sep="_"),".png",sep="")
      ...
      if (fileFormat=="eps") {
        fileLLfitsName <- paste(paste(paste(locusName,"LLfits",sep="_"),thisTrail,sep="_"),".eps",sep="")
      ...
      if (fileFormat=="png") {
        fileLLfitsName <- paste(paste(paste(locusName,"LLfits",sep="_"),thisTrail,sep="_"),".png",sep="")
      ...
      clinePlot(clineModel="symm_5p",locusName,summaryModelsAllloci,curveFit=T,cexMain=1.3,cexAxisPlot= 1.85,
                axisLineWdth=2.5,cexPoints=1.8,lwdCurve=4,refLocus=F,refPoints=F,HeaderDetails,FigLabel,HaplotypeData, colourChoice)      
      # as pdf
      fileClineFitName <- paste(paste(paste(locusName,"ClineFit",sep="_"),thisTrail,sep="_"),".pdf",sep="")
      pdf(fileClineFitName,width = 15, height = 15,pointsize=12)
      clinePlot(clineModel="symm_5p",locusName,clineFit=summaryModelsAllloci,curveFit=T,cexMain=2.5,cexAxisPlot= 2.7,
                axisLineWdth=3.8,cexPoints=3,lwdCurve=4.3,refLocus,refPoints,HeaderDetails,FigLabel,HaplotypeData, colourChoice)
      dev.off()
      # # as tiff
      # fileClineFitName <- paste(paste(paste(locusName,"ClineFit",sep="_"),thisTrail,sep="_"),".tiff",sep="")
      ...
      fileClineFitName <- paste(paste(paste(locusName,"ClineFit",sep="_"),thisTrail,sep="_"),".png",sep="")
      png(fileClineFitName,width = 1000, height = 1000, pointsize = 14,antialias="default")
      clinePlot(clineModel="symm_5p",locusName,clineFit=summaryModelsAllloci,curveFit=T,cexMain=1.4,cexAxisPlot= 2,
                axisLineWdth=2,cexPoints=1.8,lwdCurve=4,refLocus,refPoints=,HeaderDetails,FigLabel,HaplotypeData, colourChoice)
      dev.off()
      obsVals_thisLocus <- NULL
      curveFit <- NULL
    ...
    if (locusName%in%names(DemeData)==F) {
