...
# adjustXY = T/F, whether to reset the minimum X and Y spatial values to 0
...
DemeSetup2D_Phenotypes <- function(HZdata,yearsData,lociNames,colourMeans,demeSize,minSample,removePhenoMiss,transectGradient,transectIntercept,
  ...
  # HZdata <- GenoEcol_Planoles; yearsData <- c(2009:2020)
  # nrow(HZdata)
  # HZdata=HZ_Planoles;lociNames=c("Fla_2loc");colourMeans=T
  ...
  # in Easting
  # transectGradient = -0.497; transectIntercept = 8800
  ...
  # 
  # transectGradient <- 3.125; transectIntercept <- 3412000 # Cadi
  # plotDim<- "Easting";adjustXY <- FALSE; pointsCol <- FALSE; pieRadius <- 60; plotDem <- c(2,2); addPie <- T; verbose=T; poolSeqPlot=T; noPlots<-F
  # 
  validPhenoCats <- c("W","WO","WR","FR","FO","Y")
  
  HZdata <- HZdata %>%
    filter(FirstYear %in% yearsData) %>%
    filter(phenoCat_final != "-9") %>%
    filter(phenoCat_final %in% validPhenoCats)
  
  nrow(HZdata)
  table(HZdata$phenoCat_final)
  ...
  for (thisLocus in 1:length(lociNames)) {
    ...
    # converting transect intercept to Northing position
    transectIntercept_Northing <- transectIntercept + min(Yboundaries_midpoint)
    # combine
    ...
    if (noPlots==F) {
      ...
      plot(DemeMatrix2D_combine_noZero[,"X_adj"],DemeMatrix2D_combine_noZero[,"Y_adj"],type='n',
           ...
           xlim=c(xMin-2000,24300),
           ylim=c(yMin-4000,10500))
      axis(1,at=seq(xMin-2000,xMax,1000),labels=NA,col.axis="black",las=2,cex.axis=1.1,lwd=1.6,pos=yMin-4000, tck = -.02)
      axis(1,at=seq(xMin-2000,xMax,2000),labels=seq(xMin-2000,xMax,2000)/1000,col.axis="black",tck=-0.3,las=1,cex.axis=1.1,lwd=0,pos=yMin-3600)
      ...
      xMaxLine <- max(seq(xMin-2000,xMax,2000))
      ...
      if (plotEastNorthing==T) {
        ...
        axis(2,at=seq(-2246,10000,2000),labels=seq(4682000,4694000,2000),col.axis="black",tck=-.03,las=1,cex.axis=1.1,lwd=0,pos=xMaxLine+3000)
      }
        ...
        if (addPie==F) {
          points(DemeMatrix2D_combine_noZero[,"X"],DemeMatrix2D_combine_noZero[,"Y"])
        }
      }
    ...
    if ("pdf" %in% plotType) {
      ...
      pdf(fileDemes,width = 22, height = 14,pointsize=18)
      par(mfrow=c(1,1))
      par(mar=c(3,3,3,3))
      par(oma=c(3,3,3,3))
      #tempData <- HZ_Planoles_200mDemes$ros_assembly_543443$DemeMatrix2D_combine_noZero
      plot(DemeMatrix2D_combine_noZero[,"X_adj"],DemeMatrix2D_combine_noZero[,"Y_adj"],type='n',
           main="",cex.main=1,bty="n",yaxt="n",xaxt="n",
           xlab = "",ylab = "",col = "black", bg = "black",xaxs = "i", yaxs = "i",
           xlim=c(xMin-2000,24000),
           ylim=c(yMin-4000,10000))
      axis(1,at=seq(xMin-2800,xMax,1000),labels=NA,col.axis="black",las=2,cex.axis=1.1,lwd=1.6,pos=yMin-4000, tck = -.02)
      axis(1,at=seq(xMin-2800,xMax,2000),labels=seq(xMin-2800,xMax,2000)/1000,col.axis="black",tck=-0.3,las=1,cex.axis=1.1,lwd=0,pos=yMin-4000)
      axis(2,at=seq(yMin-4000,yMax+3800,1000),labels=NA,col.axis="black",las=1,cex.axis=1.1,lwd=1.6,pos=xMin-2000,tck = -.02)
      axis(2,at=seq(yMin-4000,yMax+3800,2000),labels=seq(yMin-4000,yMax+3800,2000)/1000,col.axis="black",tck=-.03,las=1,cex.axis=1.1,lwd=0,pos=xMin-2000.5)
      xMinLine <- min(seq(xMin-2800,xMax,2000))
      xMaxLine <- max(seq(xMin-2800,24000,2000))
      yMinLine <- min(seq(yMin-4000,yMax+3800,1000))
      yMaxLine <- max(seq(yMin-4000,yMax+3800,1000))
      lines(x=c(xMaxLine,xMaxLine),y=c(yMinLine,yMaxLine),lwd=1.6)
      ...
      curve(((transectGradient*x)+(transectIntercept)), from=c(-2000),to=24000, col="darkgray",add = TRUE,type = "l",lwd=2,lty=1)
      title(paste(thisLocusName,paste(200,"m demes",sep=""),sep=", "),line=2.3,cex.main=0.9)
      mtext("East (km) / Easting",side=1,cex=1,line=2.5)
      mtext("North (km) / Northing",side=2,cex=1,line=2.5)
      axis(1,at=seq(-435,24000,2000),labels=NA,outer=F,col.axis="black",las=2,cex.axis=1.1,lwd=1.6,pos=yMaxLine, tck = 0.02)
      axis(1,at=seq(-435,24000,2000),labels=seq(410000,435435,2000),col.axis="black",tck=-0.3,las=1,cex.axis=1.1,lwd=0,pos=yMaxLine+1200)
      axis(2,at=seq(-2246,10000,2000),labels=NA,col.axis="black",las=1,cex.axis=1.1,lwd=1.6,pos=xMaxLine,tck = 0.02)
