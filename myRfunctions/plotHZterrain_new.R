...
plotTerrainPiePlot_new <- function(DataSet,DataSetName,DEM_merged,SNPsFullData,locusToPlot,SNPlociListUpdated,minNum,xRange,
                               yRange,alphaTerm,colPallete,colPalleteNum,poolsAdd,demeSize,plotPies,pieRadius,
                               pointsInd,ptSize,gridAdd,landMarks,fileFormat,transectParams,markRoads=F) {
  ...
  # colnames(DataSet)
  # locusToPlot <- "hybrid.index"
  # alphaTerm <- 0.8; ptSize <- 0.7; colPallete <- land.pal; colPalleteNum <- 50; pieRadius <- 80; demeSize <- 100
  # pointsInd <- "no"; gridAdd <- FALSE; landMarks <- FALSE; fileFormat <- "pdf"; thislocusName <- "ros_assembly_543443"
  # DataSet <- "Clines_Cadi"
  # minNum <- 10; markRoads <- F; plotPies <- T; poolsAdd <- T
  # transectParams <- c(-0.22,5500) # gradient and intercept
  # transectGradient = -0.22, transectIntercept = 5500
  ...
  # plot transect
  #-0.22, 4690000
  #curve(((-0.22*x)+(4690000)), from=410000,
   #     to=430000, col="darkgray",add = TRUE,type = "l",lwd=2,lty=1)
  # Get allele frequency data
  
  
  if (locusToPlot!="hybrid.index") {
    colnames(DataSet)[1:2] <- c("X","Y")
    DataSet[,locusToPlot] <- as.numeric(DataSet[,locusToPlot])
    keepMe <- which(!is.na(DataSet[,locusToPlot]))
    thisDemeMatrix2D_pqCountNoZero <- DataSet[keepMe,]
    locX <- thisDemeMatrix2D_pqCountNoZero[,"X"]
    locY <- thisDemeMatrix2D_pqCountNoZero[,"Y"]
  ...
  # plotting pie charts
  if (locusToPlot!="hybrid.index") {
    totalN <- (thisDemeMatrix2D_pqCountNoZero[,paste0("X.",locusToPlot)])
    keepMinSize <- which(totalN>=minNum)
    thisDemeMatrix2D_pqCountNoZero <- thisDemeMatrix2D_pqCountNoZero[keepMinSize,]
  }
  if (locusToPlot=="hybrid.index") {
    totalN <- (thisDemeMatrix2D_pqCountNoZero[,paste0("X.","ros")])
    keepMinSize <- which(totalN>=minNum) 
    thisDemeMatrix2D_pqCountNoZero <- thisDemeMatrix2D_pqCountNoZero[keepMinSize,]
  }
  ...
  if (plotPies==T) {
    for (thisDeme2D in 1:nrow(thisDemeMatrix2D_pqCountNoZero)) {
      ...
      thisP <- as.numeric(as.vector(thisDemeMatrix2D_pqCountNoZero[thisDeme2D,locusToPlot]))
      thisQ <- 1-thisP
      #cat("\n .",thisDeme2D)
  ...
  if (poolsAdd==TRUE) {
    ...
    points(x=411604.6046997,y=4690386.0014007,cex=2.2,pch=22,bg='lightgreen',col='black') # YP4 - pool 1
    points(x=421903.0320033,y=4686484.3909220,cex=2.2,pch=22,bg='lightgreen',col='black') # YP - pool 2
    points(x=422119.7797325,y=4686390.2520109,cex=2.2,pch=22,bg='lightgreen',col='black') # YP - pool 3
    points(x=424462.3465657,y=4686112.3149515,cex=2.2,pch=22,bg='lightgreen',col='black') # MP - pool 4
    points(x=425146.6755758,y=4686021.5773514,cex=2.2,pch=22,bg='lightgreen',col='black') # MP - pool 5
    points(x=431621.6848923,y=4686847.1448457,cex=2.2,pch=22,bg='lightgreen',col='black') # MP - pool 6
  }
  dev.off()  
}